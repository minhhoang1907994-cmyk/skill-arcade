...
- **Availability**: Gemini being unavailable degrades gracefully — 4 of 5 games remain fully playable because they grade from `answer_key` in the local DB. A circuit breaker opens after 5 consecutive Gemini failures and stays open for 5 minutes. A call rejected by an open breaker is not itself counted as a failure, otherwise every retry would extend the open window indefinitely. Breaker state lives in `Rails.cache`. In production the store is Redis when `REDIS_URL` is set (Render Key Value), which is what deployment requires: Render's filesystem is ephemeral and per-instance, so the Rails default file store would drop breaker state on every deploy or spin-down and give each instance its own breaker. Without `REDIS_URL` the app still boots on the file store but logs a warning, so the degradation is visible rather than silent. `Gemini::DailyBudget` is unaffected because it counts rows in `ai_gradings` instead of using the cache (BR-37).
