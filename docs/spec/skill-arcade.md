...
- **Version**: 1.25 (BR-40 — hình đại diện tài khoản; 5 màn chơi đổi sang toàn quái vật)
...
| Member | create: game_session, session_answer, question_report; read: câu hỏi (không kèm đáp án), điểm và lịch sử của chính mình, leaderboard; update: `users.avatar` của CHÍNH MÌNH (BR-40) | Đã đăng nhập, tài khoản không bị khoá |
...
| avatar | varchar(20) | NO | `hero` | | Tên sprite làm hình đại diện (BR-40). Lưu TÊN, không lưu file — pixel art nằm trong code |
...
| step_served_at | datetime | YES | NULL | | Mốc câu đang chờ trả lời được phát ra cho client, là mốc đo `elapsed_ms` của BR-21. Ghi khi client xin câu để hiển thị, và chỉ ghi cho lần phát ĐẦU TIÊN của mỗi bước (F5 giữa bước không reset đồng hồ tốc độ). Xoá về NULL khi bước được trả lời. NULL nghĩa là câu đang chờ chưa phát ra lần nào |
...
| GET | `/guide` | Guest | Trang hướng dẫn (BR-41) — giới thiệu app và luật + cách tính điểm của từng game |
| GET | `/privacy` | Guest | Trang chính sách riêng tư (Q7) — phải đọc được trước khi đăng ký |
| GET | `/settings` | Member | Trang cài đặt tài khoản — hiện chỉ có chọn hình đại diện (BR-40) |
| PATCH | `/settings` | Member | Lưu hình đại diện của chính mình |
...
Response **không kèm bước tiếp theo**. Client hiện phần giải thích rồi chờ người chơi bấm
"Câu tiếp theo", và chỉ khi đó mới gọi `GET /api/v1/sessions/:id/current` để lấy câu mới.
Phát đề sớm hơn thời điểm hiển thị làm mốc đo `elapsed_ms` của BR-21 lệch: thời gian đọc
giải thích của câu trước bị cộng vào câu sau.
...
| 422 | `NO_QUESTIONS_AVAILABLE` | Ngân hàng câu hỏi không đủ câu chưa ẩn để phục vụ bước đang cần | "Chưa đủ câu hỏi cho game này". Áp cho **cả ba** endpoint bốc đề: tạo lượt, `GET /sessions/:id/current`, và nộp đáp án. Rescue nằm tập trung ở `Api::V1::BaseController` từ 1.20 — trước đó chỉ endpoint tạo lượt xử lý `Questions::Drawer::NotEnoughQuestions`, nên admin ẩn câu giữa lúc có người đang chơi (BR-16/BR-18) làm hai endpoint kia trả **500** |
...
- **BR-20**: For Estimate Poker, `actual_hours` is fixed in `answer_key` at question generation time. It is never recomputed at play time, so all players are graded against the same value. Đơn vị là **giờ làm việc của một dev đã quen codebase**, gồm điều tra + gõ code + tự viết test + sửa sau review, **không** gồm QA, họp, chờ review, deploy hay buffer rủi ro. Đơn vị này được nhắc ngay cạnh ô nhập ở giao diện, vì người chơi ước lượng theo một thang mà đề chấm theo thang khác thì điểm không nói lên điều gì. Khoảng hợp lệ là `Question::ESTIMATE_HOURS_RANGE` (0.5–80 giờ): `Questions::Validator` loại đề nằm ngoài khoảng này ngay lúc import, và prompt trong `Questions::Generator` nêu cùng định nghĩa để đề AI sinh không trôi sang thang khác.
- **BR-20b**: `answer_key.breakdown` là bảng bắt buộc gồm các dòng `{ step, hours }` — từng thao tác cụ thể và số giờ của nó. Màn hình kết quả hiện bảng này kèm dòng tổng, vì một con số trần trụi không cho người chơi biết mình ước lượng thiếu bước nào. **Tổng các dòng phải bằng `actual_hours`**: người chơi bị chấm theo `actual_hours` nên một bảng cộng ra số khác là màn hình kết quả tự mâu thuẫn với điểm nó vừa đưa ra. `Questions::Validator` loại đề sai tổng ngay lúc import; `Questions::Generator` không hỏi model `actual_hours` mà tự cộng từ breakdown, nên đề AI sinh không thể lệch. Mỗi dòng chỉ được tính việc CÓ TRONG `task_description`/`context` — thêm dòng cho việc đề không nhắc tới là trừ điểm người chơi vì một phạm vi họ không được cho biết. `Scoring::EstimatePoker` bỏ bảng (chỉ hiện phần chữ) khi đề cũ chưa có breakdown hoặc tổng lệch, thay vì hiện bảng mâu thuẫn với điểm.
- **BR-21**: Bug Hunt speed multiplier is applied on `elapsed_ms` measured server-side (time between the question being served and the answer arriving — the serving moment is `game_sessions.step_served_at`, stamped when the client requests the question for display, **not** when the previous answer was submitted): below 30s ×1.0, 30–60s ×0.8, above 60s ×0.5. The multiplier is applied **once, to the question's total raw points**, and the result is then floored — not applied to each component separately. Example: correct line and correct type at 45s gives `floor((6 + 4) × 0.8) = 8`, not `floor(6×0.8) + floor(4×0.8) = 7`.
...
- **BR-41**: A `/guide` page states, per game, the mechanic and the exact scoring breakdown, and is reachable from the header nav on every page — for guests too, so the rules can be read before registering. Step counts and score caps are read from `games` rather than written into the copy, and a game with `active = false` is left out entirely so no one reads rules for a game they cannot enter. The scoring numbers themselves live in `app/services/scoring/` with no DB representation, so changing a scorer means changing this page in the same commit.
...
- **BR-39**: Thứ tự các phương án hiển thị cho người chơi được đảo ở tầng payload (`GameSessions::StepProvider#display_content`) chứ không lấy nguyên thứ tự trong `content`: `clarifying_options` của Spec Detective, và `options` của từng node ở hai game kịch bản (Escape Room, PROD Roulette). Đề trong ngân hàng để phương án tốt nhất ở vị trí đầu, nên giữ nguyên thứ tự đó là bấm ô đầu tiên ăn điểm mà không cần đọc đề. Đảo lúc hiển thị chứ không sửa từng đề: `content` đi vào `checksum` nên sửa đề là tạo bản ghi trùng, và luật này áp luôn cho đề do `rake questions:generate` sinh về sau. Phép đảo dùng seed `(session_id, position)` giống BR-36 nên tải lại trang giữa bước vẫn thấy đúng thứ tự đó. Chấm điểm không đổi vì mọi scorer tra theo `key` của phương án, và giao diện không in `key` ra cho người chơi thấy — hệ quả: phản hồi của Spec Detective nhắc lại **nội dung** phương án tốt nhất thay vì nói "phương án a".

- **BR-40**: Mỗi tài khoản có một hình đại diện (`users.avatar`), mặc định `hero`. Giá trị hợp lệ là danh sách trong `Avatar::CHOICES` — **mọi** sprite app đang có, không riêng nhân vật: quái vật, rương báu, đồng xu, bình thuốc đều chọn được. Người chơi tự đổi ở `/settings` và chỉ đổi được của chính mình; admin KHÔNG có đường đổi hình của người khác. Cột lưu tên sprite chứ không lưu file, nên giá trị lạ sẽ làm mọi trang có avatar nổ `KeyError` — chặn hai lớp: validation `inclusion` ở model, và `Avatar.resolve` rơi về `hero` khi hiển thị (dành cho bản ghi cũ trỏ sprite đã đổi tên). Hình xuất hiện ở header, trang chọn màn, và cạnh tên trên bảng xếp hạng; vì bảng xếp hạng cache 60 giây (`LeaderboardQuery::CACHE_TTL`) nên hình mới có thể chậm tối đa 60 giây mới hiện ở đó. Mỗi màn chơi cũng có hình đại diện riêng nhưng đó là hằng số trong code (`PixelArtHelper::GAME_SPRITES`, cả 5 màn là quái vật), không phải dữ liệu và không ai đổi được qua UI.
...
- **BR-28** (Estimate Poker): 10 tasks × 10 points based on deviation from `actual_hours` — within 10% scores 10, within 25% scores 7, within 50% scores 4, beyond 50% scores 0. Sai số tính trên cùng đơn vị đã định nghĩa ở BR-20.
...
- **Hộp thoại**: mọi xác nhận, thông báo và ô nhập nhanh dùng modal dialog dùng chung (`app/views/shared/_dialog.html.erb`, `window.appDialog.alert/confirm/prompt`), **không** dùng `alert()`/`confirm()`/`prompt()` của trình duyệt — chúng không theo được style và không đặt được nhãn nút theo ngữ cảnh. Dựng trên thẻ `<dialog>` + `showModal()` để có focus trap, đóng bằng Esc và inert phần còn lại của trang mà không phải tự cài. Nút đồng ý của hành động phá huỷ dùng `.btn--danger` và đứng SAU nút huỷ trong DOM, nên Enter không kích hoạt hành động phá huỷ. Nút phá huỷ dạng form (trang admin) khai `data-confirm` — **không** `data-turbo-confirm`: Turbo không được nạp ở app này (không có `config/importmap.rb`, layout không gọi `javascript_importmap_tags`) nên `data-turbo-confirm` là code chết, và nút xoá tài khoản trước 1.19 xoá ngay khi bấm mà không hỏi gì
...
- [x] Admin ẩn câu giữa lúc có người đang chơi → `GET current` và nộp đáp án đều trả `422 NO_QUESTIONS_AVAILABLE`, lượt giữ `in_progress` và không ghi `session_answers`, người chơi thoát bằng nút Bỏ lượt (`user_quit`)
...
|---|---|---|---|
| 1.27 | 2026-08-20 | HoangNM | **BR-41**: thêm trang `/guide` và nút "Hướng dẫn" ở header mọi trang (guest xem được). Trang nói cơ chế + cách chia điểm của từng game lấy từ chính code chấm, luật chung (chấm ở server, cộng dồn, bỏ lượt không tính điểm, chính sách bốc đề BR-32, hạn mức), và cách xếp hạng kể cả tie-break theo số lượt. Số bước và trần điểm đọc từ bảng `games`, game `active = false` bị bỏ khỏi trang. Thêm style cho `h3` — trước đó chưa trang nào dùng thẻ này nên nó rơi về font mặc định của trình duyệt |
| 1.26 | 2026-08-20 | HoangNM | Sửa hai lỗi phát hiện khi chơi thật. (1) **Cùng một đáp án đúng của Bug Hunt nhận điểm khác nhau**: `elapsed_ms` phía server được đo từ `answered_at` của câu TRƯỚC, nhưng client giữ phần giải thích trên màn hình tới khi người chơi bấm "Câu tiếp theo" — nên thời gian đọc giải thích bị cộng vào đồng hồ của câu sau và hệ số BR-21 tụt xuống 0.8/0.5 dù trả lời rất nhanh. Thêm cột `game_sessions.step_served_at`, bỏ trường `next` khỏi response `POST /answers` và cho client xin câu mới qua `GET /current` đúng lúc hiển thị; mốc chỉ ghi cho lần phát đầu tiên nên F5 không reset được đồng hồ. (2) **Nút Huỷ của modal dialog dạng prompt không đóng được**: dialog đặt `required` lên textarea còn nút Huỷ là `type="submit"`, nên constraint validation chặn luôn — thêm `formnovalidate`. Gặp ở luồng "Báo câu hỏi sai" |
| 1.25 | 2026-08-20 | HoangNM | **BR-40**: mỗi tài khoản chọn được hình đại diện. Cột `users.avatar` (varchar(20), NOT NULL, default `hero`), trang `/settings` (GET + PATCH), hiển thị ở header / trang chọn màn / bảng xếp hạng. Danh sách hình hợp lệ nằm ở `Avatar::CHOICES` — một nguồn duy nhất cho cả validation và lưới chọn, và `spec/models/avatar_spec.rb` chốt nó không lệch với `PixelArtHelper::SPRITES` theo cả hai chiều. Kèm theo: 5 màn chơi đổi sang **toàn bộ là quái vật** (`spec_detective` pháp sư → quái mắt, `estimate_poker` rương báu → rương quái), thêm 2 sprite `eye` và `mimic`. **Một lỗi tự gây và đã sửa trong cùng lần này**: `LeaderboardQuery` cache chính object `Entry`, nên thêm thành viên `avatar` vào Struct làm code mới đọc payload cũ và ném `TypeError: struct size differs` → cả trang xếp hạng 500 cho tới khi cache hết hạn (60s). Test không bắt được vì test luôn `Rails.cache.clear`; chỉ mở trang thật mới thấy. Sửa bằng cách ghim chữ ký `Entry.members` vào cache key (`ENTRY_VERSION`) — tính từ chính Struct thay vì hằng số đếm tay, vì hằng số chỉ đúng khi có người nhớ tăng nó |
| 1.24 | 2026-08-20 | HoangNM | **BR-39**: đáp án đúng không còn nằm cố định ở ô đầu tiên. Đề viết tay trong `db/seeds/sample_questions.rb` xếp phương án tốt nhất lên đầu ở cả ba game có danh sách phương án (Spec Detective khoá `a`, PROD Roulette `s*a`, Incident Escape Room `e*a`), nên chơi bằng cách bấm ô đầu tiên là ăn gần trọn điểm mà không cần đọc đề. Sửa ở `GameSessions::StepProvider#display_content` — đảo thứ tự lúc dựng payload, seed theo `(session_id, position)` như BR-36 để tải lại trang giữa bước không đổi thứ tự — thay vì sửa lại từng đề: `content` đi vào `checksum` nên sửa đề sẽ tạo bản ghi trùng khi seed lại, và cách này áp luôn cho đề `rake questions:generate` sinh sau này. Điểm không đổi vì scorer tra theo `key`. Kéo theo: phản hồi của `Scoring::SpecDetective` đổi từ "phương án a" sang nhắc lại nội dung phương án đúng — sau khi đảo thứ tự thì chữ `a` không còn chỉ được vào ô nào, mà `key` cũng chưa bao giờ hiện trên giao diện |
| 1.23 | 2026-08-20 | HoangNM | `ci.yml` chỉnh cho khớp project — trước đó nó là file mẫu Rails chưa từng được sửa, và vì trigger sai branch nên không ai thấy. Bóc được 4 lỗi thật sau khi `bin/*` đã chạy được: (1) job `test`/`system-test` đặt `DATABASE_URL: trilogy://` làm Rails nạp adapter `trilogy` không có trong bundle → `LoadError`; sửa bằng cách bỏ `DATABASE_URL` và đặt `DB_*` ở cấp job đúng như `config/database.yml` đọc — scheme của URL quyết định adapter nên đây không phải chuyện đổi host. (2) job chạy `bin/rails test` = Minitest trong khi project dùng RSpec → đổi sang `bundle exec rspec`, trước đó 179 spec **không hề được chạy trên CI**. (3) job `scan_js` gọi `bin/importmap audit` mà file đó không tồn tại (gem importmap-rails/turbo/stimulus có trong Gemfile nhưng chưa wire: không có `config/importmap.rb`, không có `app/javascript`, layout không gọi `javascript_importmap_tags`) → xoá job. (4) job `system-test` chạy `test:system` nhưng không có `spec/system` nào → xoá job. Kèm theo: brakeman exit 3 vì `permit!` ở `answer_params` — **sửa thật thay vì ignore**, thay bằng `permit` liệt kê đúng 6 khoá mà các scorer đọc (`line`, `bug_type`, `hours`, `option_key`, `node_key`, `statement_indexes: []`), khớp quy tắc strong parameters của `CLAUDE.md`; `.except("score", "_meta")` bỏ được vì hai khoá đó giờ tự bị loại. Brakeman báo `No warnings found` Kèm: ghim `image: mysql:8.4` cho service của CI thay vì `mysql` (= tag `latest`, đang là 9.x) — tag latest làm job `test` đỏ với `Mysql2::Error: FUNCTION skill_arcade_test.MD5 does not exist`, vì `Questions::Drawer` dùng `MD5(CONCAT(...))` để thực thi BR-36. 8.4 khớp `docker-compose.yml` và Aiven 8.4.8 trên production. **Rủi ro chưa xử lý**: nâng MySQL production lên 9.x sẽ làm `Drawer` vỡ và BR-36 mất bảo đảm — phải verify trước khi nâng |
| 1.22 | 2026-08-20 | HoangNM | Job refill lần chạy đầu dừng với **exit code 126** — "command found but not executable". Nguyên nhân: repo tạo trên Windows nên MỌI file `bin/*` vào git với mode `100644`, thiếu bit thực thi. Đường Docker không lộ vì `Dockerfile` có `RUN chmod +x bin/*`; chỉ runner GitHub Actions chạy thẳng từ checkout mới gặp. Sửa: `git update-index --chmod=+x` cho 10 file trong `bin/`, kèm guard `spec/bin_executable_spec.rb` kiểm mode trong **git index** (không dùng `File.executable?` — trên Windows filesystem không mang bit +x nên kết quả vô nghĩa); đã verify guard đỏ đúng khi bỏ bit rồi mới phục hồi. `ci.yml` sửa trigger `push: branches` từ `master` sang **`main`**: default branch là `main` nên CI không chạy cho **bất kỳ push nào** — trigger `pull_request` thì vẫn hoạt động và đã fail trên PR Dependabot 2026-08-19, nhưng không ai đọc kết quả. Chính CI lẽ ra phải bắt lỗi này vì cũng gọi `bin/brakeman`, `bin/rubocop`, `bin/rails`. Lần chạy đầu trên `main` xác nhận: **cả 5 job** fail với `Permission denied` / exit 126, nên các nghi vấn khác về `ci.yml` (`DATABASE_URL: trilogy://` trong khi Gemfile chỉ có `mysql2`; `bin/rails ... test` chạy Minitest trong khi project dùng RSpec; brakeman exit 3 vì 1 warning `permit!` có sẵn) **chưa quan sát được** và phải kiểm lại sau khi mode đã đúng. Cùng lúc sửa `Questions::Refiller`: mục tiêu bị lượt `in_progress` chặn được lọc TRƯỚC khi chọn thay vì sau — đo trên production thấy cả 6 mục tiêu thiếu đề đều có lượt đang mở nên một mục tiêu bị chặn làm cả lần chạy sinh 0 đề mà vẫn báo xanh; mục tiêu bị chặn giờ vẫn được in ra để phân biệt "đủ đề rồi" với "lúc nào cũng có người chơi" |
| 1.21 | 2026-08-20 | HoangNM | Vá lỗ **500** ở hai endpoint: `Questions::Drawer::NotEnoughQuestions` chỉ được rescue ở endpoint tạo lượt, nên khi admin ẩn câu giữa lúc có người đang chơi (BR-16/BR-18) thì `GET /sessions/:id/current` và nộp đáp án để exception lọt ra ngoài. Rescue chuyển về tập trung ở `Api::V1::BaseController` cho cả `NotEnoughQuestions` và `StepProvider::NoQuestionAvailable`, ba rescue inline trùng nhau bị bỏ. **Sửa một điều tôi ghi sai ở phân tích ban đầu**: exception thoát ra KHÔNG phải `NoQuestionAvailable` như đã nêu — `Drawer#call` luôn trả đúng `count` câu hoặc ném `NotEnoughQuestions` trước đó, nên `fresh_question` không bao giờ hết ứng viên và `NoQuestionAvailable` hiện không chạm được; test viết theo giả định sai đó đã fail và chỉ ra đúng chỗ. Lượt cố ý GIỮ `in_progress` thay vì tự chuyển `abandoned`: đó là thay đổi state machine §11 và `abandoned_reason` chưa có giá trị nào đúng cho tình huống này — người chơi thoát bằng nút Bỏ lượt sẵn có (`user_quit`). 5 test hồi quy ở `spec/requests/gameplay_spec.rb`, gồm cả ranh giới `INVALID_LANGUAGE` vs `NO_QUESTIONS_AVAILABLE` khi ẩn hết câu làm ngôn ngữ mất khỏi bank |
| 1.20 | 2026-08-20 | HoangNM | Sửa hai lỗi phát hiện khi chơi thật trên Render sau 1.19. (1) **Spec Detective báo "Không bắt đầu được lượt chơi"**: đề format cũ vẫn `playable` nên `Questions::Drawer` vẫn bốc (đo được 6/8 lượt), và `renderSpecDetective` ném `TypeError` ở `c.statements.forEach`. Lỗi đó bị chính `try` của lời gọi API bắt nên hiện ra như lỗi server, che mất nguyên nhân thật. Ba thay đổi: `renderStep` chuyển ra NGOÀI `try` của API (lượt đã tạo ở server rồi thì lỗi dựng giao diện không phải lỗi tạo lượt), guard trong `renderSpecDetective` báo rõ "câu hỏi ở định dạng cũ" và disable nút Nộp thay vì hiện màn hình trống, và task mới `rake questions:hide_invalid` ẩn mọi đề không qua `Questions::Validator` (ẩn chứ không xoá — `session_answers` trỏ tới câu đó và BR-16 yêu cầu lượt cũ giữ điểm). (2) **Nút xoá tài khoản ở trang admin chưa bao giờ hỏi xác nhận**: nó khai `data-turbo-confirm` nhưng Turbo không được nạp ở app này — không có `config/importmap.rb`, layout không gọi `javascript_importmap_tags` — nên xoá ngay khi bấm. Thay toàn bộ `alert`/`confirm`/`prompt` của trình duyệt bằng modal dialog dùng chung `shared/_dialog.html.erb` (thẻ `<dialog>` + `showModal()`, có focus trap và Esc sẵn), và nút phá huỷ dạng form đổi sang `data-confirm` do handler của dialog đọc. Quy tắc ghi vào §13 |
